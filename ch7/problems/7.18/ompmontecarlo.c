...
long long int number_of_points_in_circle = 0;
...
int main(int argc, char *argv[]) {
  ...
  long long int number_of_all_points = atoi(argv[1]);
...
#pragma omp parallel for reduction(+:number_of_points_in_circle)
  for (long long int i = 0; i < number_of_all_points; i++) {
  ...
  printf("With %lli random points and %lli points in circle, the π is: %f\n", number_of_all_points, number_of_points_in_circle, pi);
