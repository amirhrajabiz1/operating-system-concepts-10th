...
{
    int num_commands = 0;
    while (pipe_commands[num_commands] != NULL)
    {
        num_commands++;
    }
    ...
    for (int i = 0; i < num_commands; i++)
    {
        ...
        parse_input(pipe_commands[i], command_args, &background, NULL, NULL);

        int pid = fork();
        if (pid == 0)
        {
            if (i != 0) // Not the first command, use the read end of the previous pipe as stdin
            {
                dup2(pipefds[(i - 1) * 2], 0);
            }

            if (i != num_commands - 1) // Not the last command, use the write end of the current pipe as stdout
            {
                dup2(pipefds[i * 2 + 1], 1);
            }

            for (int j = 0; j < 2 * (num_commands - 1); j++) // Close all pipe file descriptors
            {
                close(pipefds[j]);
            }
    ...
    for (int j = 0; j < 2 * (num_commands - 1); j++) // Close all pipe file descriptors in the parent process
    {
        close(pipefds[j]);
    }

    for (int i = 0; i < num_commands; i++) // Wait for all child processes to finish
    {
        int status;
        wait(&status);
    }
