...
void parse_input(char *input, char **args, int *background, char **input_file, char **output_file, char ***pipe_args);
...
void execute_previous_command(char **args, char **history, int *history_countchar, char *input_file, char *output_file, char **pipe_args);
// Executes the previous command from the command history.

void execute_command(char **args, int background, char *input_file, char *output_file, char **pipe_args);
...
int main(void)
{
    ...
    while (should_run)
    {
        ...
        parse_input(input, args, &background, &input_file, &output_file, &pipe_args); // Parse the user input into individual arguments
	...
	else if (strcmp(args[0], "history") == 0) // Check if user wants to view the shell history
	{
	  ...
	  continue;
        ...
        execute_command(args, background, input_file, output_file, pipe_args); // Execute the user command
    }
...
void parse_input(char *input, char **args, int *background, char **input_file, char **output_file, char ***pipe_args)
{
    ...
    *pipe_args = NULL;
    ...
    while (token != NULL)
    {
        ...
        else if (strcmp(token, "|") == 0) // Check for the pipe symbol
        {
            args[i] = NULL;
            *pipe_args = &args[i + 1];
        }
...
void execute_previous_command(char **args, char **history, int *history_count, char *input_file, char *output_file, char **pipe_args)
{
    if (*history_count > 0) // Check if there are commands in the history
    {
        ...
        parse_input(previous_command, args, &background, &input_file, &output_file, &pipe_args); // Parse the previous command into arguments
        execute_command(args, background, input_file, output_file, pipe_args); // Execute the previous command
...
void execute_command(char **args, int background, char *input_file, char *output_file, char **pipe_args)
{
    ...
    {
        if (pipe_args != NULL) // If the pipe symbol was found, create a pipe and fork new processes
        {
            int num_pipes = 1; // Variable to store the number of pipes
            char ***commands = (char ***)malloc(sizeof(char **) * (num_pipes + 1)); // Array to store commands

            // Set the first command as the initial arguments
            commands[0] = args;

            // Loop through the pipe arguments and split them into separate commands
            int i = 0;
            while (pipe_args[i] != NULL) {
                if (strcmp(pipe_args[i], "|") == 0) {
                    num_pipes++;
                    commands = (char ***)realloc(commands, sizeof(char **) * (num_pipes + 1));
                    commands[num_pipes - 1] = &pipe_args[i + 1];
                    pipe_args[i] = NULL;
                }
                i++;
            }

            int prev_pipefd[2]; // File descriptors for previous pipe
            int curr_pipefd[2]; // File descriptors for current pipe

            for (int j = 0; j < num_pipes; j++) {
                if (j < num_pipes - 1) {
                    if (pipe(curr_pipefd) == -1) {
                        perror("pipe");
                        exit(1);
                    }
                }
                ...
                {
                    if (j > 0) {
                        // Set the input of the current command to the output of the previous command
                    ...
                    if (j < num_pipes - 1) {
                        // Set the output of the current command to the input of the next command
                        dup2(curr_pipefd[1], STDOUT_FILENO);
                        close(curr_pipefd[0]);
                        close(curr_pipefd[1]);
                    }

                    if (input_file != NULL && j == 0) {
                    ...
                    if (output_file != NULL && j == num_pipes - 1) {
                    ...
                    execvp(commands[j][0], commands[j]); // Execute the command with the given arguments
                    perror("execvp"); // Print an error message if execution fails
                    exit(1); // Exit the child process with an error status
                } else if (pipe_pid > 0) // Parent process
                {
                    if (j > 0) {
                        close(prev_pipefd[0]);
                        close(prev_pipefd[1]);
            ...
            if (output_file != NULL) // Handle output redirection
            {
                int fd = open(output_file, O_WRONLY | O_CREAT | O_TRUNC, 0644);
                if (fd == -1)
                {
 ...
 save_history_to_file(char **history, int history_count)
{
