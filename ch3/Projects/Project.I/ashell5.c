...
void parse_input(char *input, char **args, int *background, char **input_file, char **output_file, char **commands[]);
// Parses the user input into individual arguments and determines if the command should be run in the background.

void execute_piped_commands(char **commands[], int num_commands, int background, char *input_file, char *output_file);
// Executes piped commands with the specified arguments.

void add_to_history(char *input, char **history, int *history_count);
// Adds the user input to the command history array.

void execute_previous_command(char **args, char **history, int *history_countchar, char *input_file, char *output_file, char **commands[]);
...
int main(void)
{
    ...
    while (should_run)
    {
        ...
        {
            //execute_previous_command(args, history, &history_count, input_file, output_file, &commands);
            continue;
        }
	...
	char *commands[MAX_COMMANDS];
        parse_input(input, args, &background, &input_file, &output_file, &commands); // Parse the user input into individual arguments
	printf(commands);
	...
	if (num_commands > 1) // If there are multiple commands, execute them as piped commands
	{
	  execute_piped_commands(commands, num_commands, background, input_file, output_file);
	}
	else
	{
...
void parse_input(char *input, char **args, int *background, char **input_file, char **output_file, char **commands[])
{
    ...
    *output_file = NULL;
    ...
    while (token != NULL)
    {
	if (strcmp(token, "|") == 0) // Check if the argument is '|', indicating a pipe
	{
	  args[arg_index] = NULL; // Add a NULL terminator at the end of the current command
	  cmd_index++;
	  arg_index = 0;
	  commands[cmd_index] = &args[arg_index]; // Set the next command to begin after the current command
	}
	else
	{
...
void execute_previous_command(char **args, char **history, int *history_count, char *input_file, char *output_file, char **commands[])
{
    if (*history_count > 0) // Check if there are commands in the history
    {
        ...
        parse_input(previous_command, args, &background, &input_file, &output_file, &commands); // Parse the previous command into arguments
