...
void execute_command(char **args, int background, char *input_file, char *output_file, char **pipe_args)
{
    ...
    if (pid == 0) // Child process
    {
      ...
      if (pipe_args != NULL) // If the pipe symbol was found, create a pipe and fork a new process
      {
	int pipefd[2];
	if (pipe(pipefd) == -1)
	{
	...
	if (pipe_pid > 0) // Parent process (first command)
	{
	  close(pipefd[0]);
	...
	else if (pipe_pid == 0) // Child process (second command)
	{
	  close(pipefd[1]);
