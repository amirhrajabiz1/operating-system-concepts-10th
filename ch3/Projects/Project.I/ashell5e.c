...
void handle_pipe_commands(char **pipe_commands, int background, char **history, int history_count)
{
  ...
  int num_commands = 0;
  while (pipe_commands[num_commands] != NULL) num_commands++;

  int pipe_fd[num_commands -1][2];
  ...
  for (int i = 0; i < num_commands; i++)
  {
    ...
    {
      int read_fd = (i > 0) ? pipe_fd[i - 1][0] : -1;
      int write_fd = (i < num_commands - 1) ? pipe_fd[i][1] : -1;
	...
	close(pipe_fd[j][1]);
      ...
      char *args[MAX_LINE / 2 + 1];
      parse_input(pipe_commands[i], args, &background, NULL, NULL);

      execute_pipe_command(args, read_fd, write_fd);
      exit(0);
    }
  }
